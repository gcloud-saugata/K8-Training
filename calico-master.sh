...
rm /root/.kube/config
kubeadm reset -f
kubeadm init --pod-network-cidr=10.244.0.0/16 --ignore-preflight-errors=NumCPU --skip-token-print

mkdir -p ~/.kube
sudo cp -i /etc/kubernetes/admin.conf ~/.kube/config

kubectl apply -f https://docs.projectcalico.org/archive/v3.16/manifests/tigera-operator.yaml
